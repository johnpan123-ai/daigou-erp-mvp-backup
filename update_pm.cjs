...
// 3. State
const old_state = `  const [activeTab, setActiveTab] = useState<'worksheet' | 'purchase_batches' | 'private_orders'>('worksheet');
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(new Set());
  const [expandedOrderRows, setExpandedOrderRows] = useState<Set<string>>(new Set());`;

const new_state = `  const [activeTab, setActiveTab] = useState<'worksheet' | 'purchase_batches' | 'private_orders'>('worksheet');
...
const old_loaddata_expand = `    // Expand all groups by default
...
// 5. toggleGroup
const old_toggle = `  const toggleGroup = (title: string) => {
    const next = new Set(expandedGroups);
    if (next.has(title)) next.delete(title);
    else next.add(title);
    setExpandedGroups(next);
  };`;
const new_toggle = `  const toggleGroup = (title: string) => {
...
// 6. Buttons
const old_buttons = `<button className="btn btn-outline" style={{ fontSize: '13px', padding: '6px 12px', backgroundColor: '#fff' }} onClick={() => setExpandedGroups(new Set(Object.keys(groupedVariants)))}>展開群組</button>
...
const new_buttons = `<button className="btn btn-outline" style={{ fontSize: '13px', padding: '6px 12px', backgroundColor: '#fff' }} onClick={() => setManualExpandedGroups(new Set(Object.keys(groupedVariants)))}>展開群組</button>
...
const old_single_display = `                          <td style={tdStyle('left')}>
...
const new_single_display = `                          <td style={tdStyle('left')}>
...
const old_multi_parent = `                        <td style={tdStyle('left')} onClick={() => toggleGroup(title)}>
...
const new_multi_parent = `                        <td style={tdStyle('left')} onClick={() => toggleGroup(title)}>
                          <div className="flex items-center gap-sm cursor-pointer" style={{ fontWeight: 600, color: '#111827' }}>
                            {isExpanded ? <ChevronDown size={16} className="text-muted" /> : <ChevronRight size={16} className="text-muted" />}
                            <HighlightText text={title} highlight={searchTerm} />
                            <span style={{ backgroundColor: '#e2e8f0', color: '#475569', padding: '2px 6px', borderRadius: '12px', fontSize: '11px', fontWeight: 500 }}>`;
...
// 11. Multi Item Child
const old_multi_child = `                                <div style={{ fontWeight: 500, color: '#334155' }}>{v.variant_name}</div>
                                <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px' }}>{v.myacg_item_code}</div>`;
const new_multi_child = `                                <div style={{ fontWeight: 500, color: '#334155' }}>
