...
async function main() {
  let refreshToken = "6fxeqwjxy23d"; // Start with the extracted refresh token
  ...
  if (fs.existsSync(sessionFilePath)) {
    try {
      ...
      if (savedSession && savedSession.refresh_token) {
        ...
        console.log(`Using saved refresh token: ${refreshToken}`);
      }
    } catch (e) {
  ...
  if (refreshErr) {
    console.error("Supabase login failed:", refreshErr.message);
    return;
  }

  const session = refreshData.session;
  console.log(`Logged in successfully as: ${refreshData.user.email}`);
  
  // Save new session for next runs
  fs.writeFileSync(sessionFilePath, JSON.stringify({
    refresh_token: session.refresh_token,
    access_token: session.access_token,
    user_email: refreshData.user.email,
    updated_at: new Date().toISOString()
  }, null, 2));
  console.log(`Saved new refresh token: ${session.refresh_token}`);
